...
import type { AnalysisResult, AnalysisWarning } from '@/types'

function SeverityIcon({ severity }: { severity: AnalysisWarning['severity'] }) {
  if (severity === 'critical') return <AlertTriangle size={14} className="text-accent-rose" />
  if (severity === 'warning') return <AlertTriangle size={14} className="text-accent-amber" />
  return <Info size={14} className="text-brand-400" />
}

function WarningPill({ w }: { w: AnalysisWarning }) {
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {result.insights.map((ins, i) => (
              <div key={i} className="p-3 rounded-xl bg-surface-700/40 border border-surface-600/30">
                <p className="stat-label">{ins.label}</p>
